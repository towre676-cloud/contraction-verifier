...
#
# Features:
# - Core Banach smallness check:  gamma * (1 + t * c_P) <= 1 - epsilon
# - Ledger (discrete) check:      gamma_k * (1 + t_k * c_P) * exp(-sigma * delta_tau_k) <= 1 - epsilon
# - Coupled streams (n-stream):   spectral radius rho(L) < 1, with Collatz-Wielandt enclosures
# - Epsilon from witness:         epsilon = alpha^beta * exp(-sigma * delta_tau_min), or override via --eps
# - Interval arithmetic:          propagate measurement bounds with outward rounding
# - Verified numerics:            final inequality rounded outward (nextafter) or strict margins
# - gamma_V sharpened bound:      ||R * Gamma||_2 via SVD or power iteration
...
def verify_ledger(data: Dict[str, Any], args) -> bool:
    ...
    for i, st in enumerate(steps):
        ...
        if i < 10 or not ok:
            print(f'  step {i:03d}: {lhs_adj:.9g} <= {rhs_adj:.9g}  {"\u2713" if ok else "\u2717"}')
